...
The `exposr` command is provided by the published package. An editable installation means source changes are immediately used without reinstalling the package.
...
On the relay machine, install the published package from PyPI:

```bash
python3 -m pip install exposr
```
